...
in with inputs; [
  ...
  (final: prev: {
    ...
    #  in
    #  prev.aseprite.overrideAttrs (_: {
    #    inherit (package) src version;
    #  });

    wezterm-git = let package = getPackage "wezterm" prev;
    in (prev.callPackage
      "${inputs.nixpkgs}/pkgs/applications/terminal-emulators/wezterm" {
        inherit (prev.darwin.apple_sdk.frameworks)
          Cocoa CoreGraphics Foundation System UserNotifications;
